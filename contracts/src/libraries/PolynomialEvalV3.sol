...
library PolynomialEvalV3 {
    ...
    function evaluatePiPoly(
        ...
        uint256[5] memory suffix;

        // Assume we have [a, b, c, d, e] where a = zeta - g^0, b = zeta - g^1, ...
        //
        // suffix[length - i - 1] = suffix[length - i] * (zeta - g^(length - i)) and
        // suffix[length - 1] = 1
        // suffix = [edcb, edc, ed, e, 1]
        assembly {
            let suffixPtr := add(suffix, mul(4, 0x20))
            let localDomainElementsPtr := add(mload(add(domain, 0x40)), mul(4, 0x20))
